...
// areaOfGame.appendChild(new Progress().element);
...
function colided(bird, barriers){
    ...
    barriers.pairs.forEach(pairOfBarriers => {
        if(!colided){
            ...
            colided = areOverlapping(bird.element, upper)
...
function FlappyBird(){
    ...
    const barriers = new Barriers(hght, wdth, 200, 400, () => progress.atualizePoints(++points));
    ...
    barriers.pairs.forEach(pair => areaOfGame.appendChild(pair.element));
    this.start = () => {
        const temporizator = setInterval(() => {
            barriers.animate();
            bird.animate();
